...
/// </summary>
public struct WindField_HashKey
{
    private Vector3Int[] key;

    //constructor generates the hash key
    public WindField_HashKey(Vector3 pos, float rootCellSize, uint depth) : this()
    {
    ...
    */
    ...
    {
        if (!(other is WindField_HashKey)) return false;

        return Equals((WindField_HashKey)other);
    }

    public bool Equals(WindField_HashKey other)
    {
    ...
    public static bool operator ==(WindField_HashKey l, WindField_HashKey r)
    {
        return Equals(l, r);
    }
